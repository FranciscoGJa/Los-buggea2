...
public class ServicioRecordatorios {
    ...
    private boolean enviarRecordatorioIndividual(Cita cita) {
        try {
            ...
            // Obtener el correo – primero del PerfilCitas
            String correoPaciente = cita.getPerfilCitas().getEmail();
            String nombrePaciente = cita.getPerfilCitas().getNombreCompleto();

            // Si el PerfilCitas no tiene email, intentar usar el del Paciente asociado
    ...
    public void iniciarRecordatoriosAsync() {
        new Thread(() -> {
            try {
                probarRecordatoriosHoy();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }).start();
